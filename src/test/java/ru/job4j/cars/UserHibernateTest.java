...
public class UserHibernateTest {
    ...
    public void createCustomArrayType() {
        ...
        Uuid uuid = new Uuid();
        uuid.setUuid(firstID);
        user.setUuids(Set.of(uuid));
    ...
    public void getUserByCookie() {
        ...
        secondUuid.setUuid(secondID);
        firstUuid.setUuid(firstID);
        user.setUuids(Set.of(firstUuid));
        fakeUser.setUuids(Set.of(secondUuid));
        mainRepository.tx((Consumer<Session>) session -> session.persist(user));
